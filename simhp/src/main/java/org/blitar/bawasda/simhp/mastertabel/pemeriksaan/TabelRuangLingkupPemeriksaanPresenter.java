...
public class TabelRuangLingkupPemeriksaanPresenter implements ClickListener, ValueChangeListener{
	private TabelRuangLingkupPemeriksaanModel model;
	private TabelRuangLingkupPemeriksaanView view;
	
	public TabelRuangLingkupPemeriksaanPresenter(TabelRuangLingkupPemeriksaanModel model, TabelRuangLingkupPemeriksaanView view){
		this.model = model;
		this.view = view;
	...
	public void valueChange(ValueChangeEvent event) {
		...
		if (entitySelected) {
			...
			model.itemHeader = new TabelRuangLingkupPemeriksaan();
...
//			discardForm(operationStatus);
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new TabelRuangLingkupPemeriksaan();
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getRuangLingkupPemeriksaanservice().removeObject(model.getItemHeader());
	...
	public int insertAndValidateForm(){
		...
		if (model.getRuangLingkupPemeriksaanservice().findAll(theId).size()>0){
			kodeValid = false;
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getRuangLingkupPemeriksaanservice().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getRuangLingkupPemeriksaanservice().updateObject(model.getItemHeader());
