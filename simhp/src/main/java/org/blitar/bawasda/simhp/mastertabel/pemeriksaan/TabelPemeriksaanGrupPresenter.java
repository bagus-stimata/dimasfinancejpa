...
public class TabelPemeriksaanGrupPresenter implements ClickListener, ValueChangeListener{
	private TabelPemeriksaanGrupModel model;
	private TabelPemeriksaanGrupView view;
	
	public TabelPemeriksaanGrupPresenter(TabelPemeriksaanGrupModel model, TabelPemeriksaanGrupView view){
		this.model = model;
		this.view = view;
	...
	public void valueChange(ValueChangeEvent event) {
		...
		if (entitySelected) {
			...
			model.itemHeader = new TabelJenisPemeriksaanGrup();
...
//			discardForm(operationStatus);
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new TabelJenisPemeriksaanGrup();
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getJenisPemeriksaanGrupservice().removeObject(model.getItemHeader());
	...
	public int insertAndValidateForm(){
		...
		if (model.getJenisPemeriksaanGrupservice().findAll(theId).size()>0){
			kodeValid = false;
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getJenisPemeriksaanGrupservice().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getJenisPemeriksaanGrupservice().updateObject(model.getItemHeader());
