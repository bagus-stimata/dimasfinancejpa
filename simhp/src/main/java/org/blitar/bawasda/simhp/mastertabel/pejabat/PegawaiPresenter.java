...
public class PegawaiPresenter implements ClickListener, ValueChangeListener{
	private PegawaiModel model;
	private PegawaiView view;
	
	public PegawaiPresenter(PegawaiModel model, PegawaiView view){
		this.model = model;
		this.view = view;
...
//			discardForm(operationStatus);
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new Pegawai();
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getPegawaiService().removeObject(model.getItemHeader());
	...
	public int insertAndValidateForm(){
		...
		String strGrup = "";
		PegawaiJabatan grup = new PegawaiJabatan();		
		try{
		...
		if (model.getPegawaiService().findAll(theId).size()>0){
			kodeValid = false;
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getPegawaiService().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getPegawaiService().updateObject(model.getItemHeader());
...
//		Filter filter1 = new SimpleStringFilter("refno", theRefno, true, false);
		...
		Filter filter3 = new Or(new SimpleStringFilter("name", theDesc, true, false));
//		model.getBeanItemContainerBukugiro().addContainerFilter(filter1);
		...
		if (! theGrup.trim().equals("")){			
			Filter filter4 = new Or(new Compare.Equal("pegawaiJabatanBean", grup));
