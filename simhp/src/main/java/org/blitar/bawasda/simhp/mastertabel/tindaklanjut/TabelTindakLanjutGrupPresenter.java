...
public class TabelTindakLanjutGrupPresenter implements ClickListener, ValueChangeListener{
	private TabelTindakLanjutGrupModel model;
	private TabelTindakLanjutGrupView view;
	
	public TabelTindakLanjutGrupPresenter(TabelTindakLanjutGrupModel model, TabelTindakLanjutGrupView view){
		this.model = model;
		this.view = view;
	...
	public void valueChange(ValueChangeEvent event) {
		...
		if (entitySelected) {
			...
			model.itemHeader = new TabelTindakLanjutGrup();
...
//			discardForm(operationStatus);
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new TabelTindakLanjutGrup();
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getTindakLanjutGrupservice().removeObject(model.getItemHeader());
	...
	public int insertAndValidateForm(){
		...
		if (model.getTindakLanjutGrupservice().findAll(theId).size()>0){
			kodeValid = false;
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getTindakLanjutGrupservice().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getTindakLanjutGrupservice().updateObject(model.getItemHeader());
