...
public class TabelLembagaTk2Presenter implements ClickListener, ValueChangeListener{
	private TabelLembagaTk2Model model;
	private TabelLembagaTk2View view;
	
	public TabelLembagaTk2Presenter(TabelLembagaTk2Model model, TabelLembagaTk2View view){
		this.model = model;
		this.view = view;
	...
	public void valueChange(ValueChangeEvent event) {
		...
		if (entitySelected) {
			...
			model.itemHeader = new TabelLembagaTkII();
...
//			discardForm(operationStatus);
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new TabelLembagaTkII();
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getLembagaTk2Service().removeObject(model.getItemHeader());
	...
	public int insertAndValidateForm(){
		...
		String strGrup = "";
		TabelLembagaTkI grup = new TabelLembagaTkI();		
		try{
		...
		if (model.getLembagaTk2Service().findAll(theId).size()>0){
			kodeValid = false;
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getLembagaTk2Service().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getLembagaTk2Service().updateObject(model.getItemHeader());
...
//		model.getBeanItemContainerBukugiro().addContainerFilter(filter1);
		...
		if (! theGrup.trim().equals("")){			
			Filter filter4 = new Or(new Compare.Equal("lembagaTk1Bean", grup));
