...
public class TabelLembagaTk1Presenter implements ClickListener, ValueChangeListener{
	private TabelLembagaTk1Model model;
	private TabelLembagaTk1View view;
	
	public TabelLembagaTk1Presenter(TabelLembagaTk1Model model, TabelLembagaTk1View view){
		this.model = model;
		this.view = view;
	...
	public void valueChange(ValueChangeEvent event) {
		...
		if (entitySelected) {
			...
			model.itemHeader = new TabelLembagaTkI();
...
//			discardForm(operationStatus);
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new TabelLembagaTkI();
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getLembagaTk1service().removeObject(model.getItemHeader());
	...
	public int insertAndValidateForm(){
		...
		if (model.getLembagaTk1service().findAll(theId).size()>0){
			kodeValid = false;
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getLembagaTk1service().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getLembagaTk1service().updateObject(model.getItemHeader());
