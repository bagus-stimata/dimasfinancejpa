...
import org.blitar.bawasda.simhp.DashboardUI;
...
public class TabelRekomendasiView extends CustomComponent{
	...
	private ComboBox comboGrup = new ComboBox("KELOMPOK");
	...
	private ComboBox comboSearchByGrup = new ComboBox("KELOMPOK");
	...
	public TabelRekomendasiView(TabelRekomendasiModel model){
		this.model = model;
		initComponent();
		buildView();
		initComponentState();
...
//		layoutTop.setComponentAlignment(fieldSearchByRefno, Alignment.BOTTOM_CENTER);
	...
	public void bindAndBuildFieldGroupComponent(){
		...
		model.getBinderHeader().bind(comboGrup, "rekomendasiGrupBean");
	...
	public void setTableProperties(){
		...
		setVisibleTableProperties("id", "description", "rekomendasiGrupBean", 
				"statusActive");
...
//		} catch(Exception ex){}
		...
		table.setColumnHeader("rekomendasiGrupBean", "GRUP");
...
//		table.setColumnExpandRatio("recapno", 3);
	...
	public void setFormButtonAndTextState(){
		...
		//USER OTORIZE HERE
		User globalUserActive = new User();
		globalUserActive = ((DashboardUI) getUI().getCurrent()).getUserActive();
		if (! globalUserActive.getUserOtorizeType().equals(EnumUserOtorize.ADMINISTRATOR.getStrCode()) || 
				! globalUserActive.getUserOtorizeType().equals(EnumUserOtorize.MANAGER1.getStrCode()) ||
				! globalUserActive.getUserOtorizeType().equals(EnumUserOtorize.USER1.getStrCode()) ){
			btnAddForm.setEnabled(false);
			btnDeleteForm.setEnabled(false);
		}
	...
	public TabelRekomendasiModel getModel() {
		return model;
	}

	public void setModel(TabelRekomendasiModel model) {
		this.model = model;
	}
