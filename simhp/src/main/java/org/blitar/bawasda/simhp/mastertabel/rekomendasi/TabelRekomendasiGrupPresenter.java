...
public class TabelRekomendasiGrupPresenter implements ClickListener, ValueChangeListener{
	private TabelRekomendasiGrupModel model;
	private TabelRekomendasiGrupView view;
	
	public TabelRekomendasiGrupPresenter(TabelRekomendasiGrupModel model, TabelRekomendasiGrupView view){
		this.model = model;
		this.view = view;
	...
	public void valueChange(ValueChangeEvent event) {
		...
		if (entitySelected) {
			...
			model.itemHeader = new TabelRekomendasiGrup();
...
//			discardForm(operationStatus);
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new TabelRekomendasiGrup();
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getRekomendasiGrupservice().removeObject(model.getItemHeader());
	...
	public int insertAndValidateForm(){
		...
		if (model.getRekomendasiGrupservice().findAll(theId).size()>0){
			kodeValid = false;
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getRekomendasiGrupservice().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getRekomendasiGrupservice().updateObject(model.getItemHeader());
