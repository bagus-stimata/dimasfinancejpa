...
public class TabelJenisKerugianPresenter implements ClickListener, ValueChangeListener{
	private TabelJenisKerugianModel model;
	private TabelJenisKerugianView view;
	
	public TabelJenisKerugianPresenter(TabelJenisKerugianModel model, TabelJenisKerugianView view){
		this.model = model;
		this.view = view;
	...
	public void valueChange(ValueChangeEvent event) {
		...
		if (entitySelected) {
			...
			model.itemHeader = new TabelJenisKerugian();
...
//			discardForm(operationStatus);
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new TabelJenisKerugian();
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getJenisKerugianService().removeObject(model.getItemHeader());
	...
	public int insertAndValidateForm(){
		...
		if (model.getJenisKerugianService().findAll(theId).size()>0){
			kodeValid = false;
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getJenisKerugianService().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getJenisKerugianService().updateObject(model.getItemHeader());
