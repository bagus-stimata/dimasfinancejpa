...
public class TabelPemeriksaanPresenter implements ClickListener, ValueChangeListener{
	private TabelPemeriksaanModel model;
	private TabelPemeriksaanView view;
	
	public TabelPemeriksaanPresenter(TabelPemeriksaanModel model, TabelPemeriksaanView view){
		this.model = model;
		this.view = view;
	...
	public void valueChange(ValueChangeEvent event) {
		...
		if (entitySelected) {
			...
			model.itemHeader = new TabelJenisPemeriksaan();
...
//			discardForm(operationStatus);
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new TabelJenisPemeriksaan();
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getJenisPemeriksaanService().removeObject(model.getItemHeader());
	...
	public int insertAndValidateForm(){
		...
		String strGrup = "";
		TabelJenisPemeriksaanGrup grup = new TabelJenisPemeriksaanGrup();		
		try{
			grup = model.getBeanItemContainerGrup().getItem(view.getComboGrup().getValue()).getBean();
			strGrup = grup.getId();
		} catch(Exception ex){}
		
		if (view.getComboGrup().getValue()==null){
			kodeValid = false;
			Notification.show("GRUP Tidak boleh Kosong!");
			view.focustIdOrDesc();
			return 1;			
		}
		...
		if (model.getJenisPemeriksaanService().findAll(theId).size()>0){
			kodeValid = false;
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getJenisPemeriksaanService().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getJenisPemeriksaanService().updateObject(model.getItemHeader());
...
//		String theRefno = view.getFieldSearchByRefno().getValue().toString().trim();
		...
		String theGrup = "";
		try{
			grup = model.getBeanItemContainerGrup().getItem(view.getComboSearchByGrup().getValue()).getBean();
			theGrup = grup.getId();		
		} catch(Exception ex){}
...
//		model.getBeanItemContainerBukugiro().addContainerFilter(filter1);
		...
		if (! theGrup.trim().equals("")){			
			Filter filter4 = new Or(new Compare.Equal("jenisPemeriksaanGrupBean", grup));
