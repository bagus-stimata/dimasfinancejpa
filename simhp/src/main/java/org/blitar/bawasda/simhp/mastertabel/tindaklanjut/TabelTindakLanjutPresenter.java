...
public class TabelTindakLanjutPresenter implements ClickListener, ValueChangeListener{
	private TabelTindakLanjutModel model;
	private TabelTindakLanjutView view;
	
	public TabelTindakLanjutPresenter(TabelTindakLanjutModel model, TabelTindakLanjutView view){
		this.model = model;
		this.view = view;
	...
	public void valueChange(ValueChangeEvent event) {
		...
		if (entitySelected) {
			...
			model.itemHeader = new TabelTindakLanjut();
...
//			discardForm(operationStatus);
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new TabelTindakLanjut();
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getTindakLanjutService().removeObject(model.getItemHeader());
	...
	public int insertAndValidateForm(){
		...
		String strGrup = "";
		TabelTindakLanjutGrup grup = new TabelTindakLanjutGrup();		
		try{
		...
		if (model.getTindakLanjutService().findAll(theId).size()>0){
			kodeValid = false;
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getTindakLanjutService().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getTindakLanjutService().updateObject(model.getItemHeader());
...
//		model.getBeanItemContainerBukugiro().addContainerFilter(filter1);
		...
		if (! theGrup.trim().equals("")){			
			Filter filter4 = new Or(new Compare.Equal("tindakLanjutGrupBean", grup));
