...
public class TabelTemuanGrupPresenter implements ClickListener, ValueChangeListener{
	private TabelTemuanGrupModel model;
	private TabelTemuanGrupView view;
	
	public TabelTemuanGrupPresenter(TabelTemuanGrupModel model, TabelTemuanGrupView view){
		this.model = model;
		this.view = view;
	...
	public void valueChange(ValueChangeEvent event) {
		...
		if (entitySelected) {
			...
			model.itemHeader = new TabelTemuanGrup();
...
//			discardForm(operationStatus);
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new TabelTemuanGrup();
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getTemuanGrupService().removeObject(model.getItemHeader());
	...
	public int insertAndValidateForm(){
		...
		String strGrup = "";
		TabelTemuanKelompok grup = new TabelTemuanKelompok();		
		try{
			grup = model.getBeanItemContainerKelompok().getItem(view.getComboGrup().getValue()).getBean();
			strGrup = grup.getId();
		} catch(Exception ex){}
		...
		if (model.getTemuanGrupService().findAll(theId).size()>0){
			kodeValid = false;
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getTemuanGrupService().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getTemuanGrupService().updateObject(model.getItemHeader());
...
//		String theRefno = view.getFieldSearchByRefno().getValue().toString().trim();
		...
		String theGrup = "";
		try{
			grup = model.getBeanItemContainerKelompok().getItem(view.getComboSearchByGrup().getValue()).getBean();
			theGrup = grup.getId();		
		} catch(Exception ex){}
...
//		model.getBeanItemContainerBukugiro().addContainerFilter(filter1);
		...
		if (! theGrup.trim().equals("")){			
			Filter filter4 = new Or(new Compare.Equal("temuanKelompokBean", grup));
