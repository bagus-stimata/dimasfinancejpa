...
public class SuratTugasTimAnggotaPosisiPresenter implements ClickListener, ValueChangeListener{
	private SuratTugasTimAnggotaPosisiModel model;
	private SuratTugasTimAnggotaPosisiView view;
	
	public SuratTugasTimAnggotaPosisiPresenter(SuratTugasTimAnggotaPosisiModel model, SuratTugasTimAnggotaPosisiView view){
		this.model = model;
		this.view = view;
	...
	public void valueChange(ValueChangeEvent event) {
		...
		if (entitySelected) {
			...
			model.itemHeader = new SuratTugasTimAnggotaPosisi();
...
//			discardForm(operationStatus);
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new SuratTugasTimAnggotaPosisi();
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getSuratTugasTimAnggotaPosisiService().removeObject(model.getItemHeader());
	...
	public int insertAndValidateForm(){
		...
		if (model.getSuratTugasTimAnggotaPosisiService().findAll(theId).size()>0){
			kodeValid = false;
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getSuratTugasTimAnggotaPosisiService().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getSuratTugasTimAnggotaPosisiService().updateObject(model.getItemHeader());
