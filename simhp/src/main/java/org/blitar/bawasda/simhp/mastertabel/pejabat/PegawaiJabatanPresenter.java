...
public class PegawaiJabatanPresenter implements ClickListener, ValueChangeListener{
	private PegawaiJabatanModel model;
	private PegawaiJabatanView view;
	
	public PegawaiJabatanPresenter(PegawaiJabatanModel model, PegawaiJabatanView view){
		this.model = model;
		this.view = view;
	...
	public void valueChange(ValueChangeEvent event) {
		...
		if (entitySelected) {
			...
			model.itemHeader = new PegawaiJabatan();
...
//			discardForm(operationStatus);
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new PegawaiJabatan();
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getPegawaiJabatanService().removeObject(model.getItemHeader());
	...
	public int insertAndValidateForm(){
		...
		if (model.getPegawaiJabatanService().findAll(theId).size()>0){
			kodeValid = false;
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getPegawaiJabatanService().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getPegawaiJabatanService().updateObject(model.getItemHeader());
