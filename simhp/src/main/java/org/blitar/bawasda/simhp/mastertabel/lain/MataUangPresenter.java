...
public class MataUangPresenter implements ClickListener, ValueChangeListener{
	private MataUangModel model;
	private MataUangView view;
	
	public MataUangPresenter(MataUangModel model, MataUangView view){
		this.model = model;
		this.view = view;
...
//			discardForm(operationStatus);
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new MataUang();
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getMataUangService().removeObject(model.getItemHeader());
	...
	public int insertAndValidateForm(){
		...
		if (model.getMataUangService().findAll(theId).size()>0){
			kodeValid = false;
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getMataUangService().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getMataUangService().updateObject(model.getItemHeader());
...
//		String theRefno = view.getFieldSearchByRefno().getValue().toString().trim();
		...
		String theDescLong = view.getFieldSearchByDescLong().getValue().toString().trim();
...
//		Filter filter1 = new SimpleStringFilter("refno", theRefno, true, false);
		...
		Filter filter3 = new Or(new SimpleStringFilter("shortName", theDesc, true, false));
		Filter filter4 = new Or(new SimpleStringFilter("fullName", theDescLong, true, false));
