...
public class TabelLembagaTk3Presenter implements ClickListener, ValueChangeListener{
	private TabelLembagaTk3Model model;
	private TabelLembagaTk3View view;
	
	public TabelLembagaTk3Presenter(TabelLembagaTk3Model model, TabelLembagaTk3View view){
		this.model = model;
		this.view = view;
	...
	public void valueChange(ValueChangeEvent event) {
		...
		if (entitySelected) {
			...
			model.itemHeader = new TabelLembagaTkIII();
...
//			discardForm(operationStatus);
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new TabelLembagaTkIII();
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getLembagaTk3Service().removeObject(model.getItemHeader());
	...
	public int insertAndValidateForm(){
		...
		String strGrup = "";
		TabelLembagaTkII grup = new TabelLembagaTkII();		
		try{
		...
		if (model.getLembagaTk3Service().findAll(theId).size()>0){
			kodeValid = false;
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getLembagaTk3Service().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getLembagaTk3Service().updateObject(model.getItemHeader());
