...
public class DivisionPresenter implements ClickListener, ValueChangeListener, Handler {
	/**
	 * 
	 */
	...
	public DivisionPresenter(DivisionModel model, DivisionView view){
		this.model = model;
		this.view = view;
		initListener();		
		initDisplay();
...
//			discardForm(operationStatus);
	...
	protected void addItem() {
		try {
			...
			//2. Tampilkan form
			model.formBeanItem = new BeanItem<Division>(item);
	...
	public int searchForm(){
		...
		Filter filter2 = new Or(new SimpleStringFilter("divisi", theName, true, false));
