...
public class SalesmanBinderPresenter implements ClickListener, ValueChangeListener, Handler {
	/**
	 * 
	 */
	...
	private SalesmanBinderView view;
	
	Salesman item = new Salesman();
	
	public SalesmanBinderPresenter(SalesmanBinderModel model, SalesmanBinderView view){
		this.model = model;
		this.view = view;
		initListener();		
		initDisplay();
	...
	public void buttonClick(ClickEvent event) {
		...
		if (event.getButton() == view.getAddButton()) {
			if (view.getOperationStatus().equals(EnumFormOperationStatus.OPEN.getStrCode())){
				addItem();
				if (view.getForm().isVisible()) view.getForm().getField("spcode").focus();
...
//                    		view.setDisplay();
	...
	protected void addItem() {
		try {
			...
			//2. Tampilkan form
			model.formBeanItem = new BeanItem<Salesman>(item);
...
//    		view.setDisplay();
	...
	public int insertAndValidateForm(){
		...
		String theId = view.getForm().getField("spcode").getValue().toString();
		//1. Validasi Kode
		...
		Equal filter = new Compare.Equal("spcode", theId);
...
//		view.setDisplay();
	...
	public int searchForm(){
		...
		Filter filter1 = new SimpleStringFilter("spcode", theId, true, false);
...
//							: item.getItemPropertyIds());
			...
			model.setItem(model.getTableJpaContainer().getItem(itemId).getEntity());
			view.setDisplay();
