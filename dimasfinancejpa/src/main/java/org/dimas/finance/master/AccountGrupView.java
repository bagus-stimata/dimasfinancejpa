...
public class AccountGrupView extends CustomComponent implements ValueChangeListener, Handler, ClickListener{
	...
	private Accountgroup accountGrup;
	private BeanItem<Accountgroup> beanItemAccountGrup;
	...
	public AccountGrupView() {
		operationStatus = EnumFormOperationStatus.OPEN.getStrCode();
		
		//Account Grup
		accountGrup = new Accountgroup();
		//Bean item accountGrup dideklarasikan hanya jika dipakai
		
		this.entityClass = Accountgroup.class;
	...
	public void initContainer(){
		container = JPAContainerFactory.make(Accountgroup.class, persistenceUnit);
...
//		setSizeFull();
		...
		setVisibleTableProperties("id","name");
		//Sementara dihandle oleh CustomFieldFactory
		//setVisibleFormProperties("id","name");
...
//			discardForm(operationStatus);
	...
	protected void addItem() {
		try {
			...
			accountGrup.setId("New");
			accountGrup.setName("");
			//2. Tampilkan form
			beanItemAccountGrup = new BeanItem<Accountgroup>(accountGrup);
			form.setItemDataSource(beanItemAccountGrup);
			form.setVisible(true);
...
//			table.setValue(itemId);
	...
	public int insertAndValidateForm(){
		...
		if (kodeValid && inputValid) {
			try {
				...
				Object itemId = container.addEntity(accountGrup);
				table.setValue(itemId);
	...
	//Menggunakan HelpOverlay	
		...
		HelpOverlay w = helpManager.addOverlay(EnumHelpOverlayTipe.SHORTCUT.getIntCode(), 
				"Account Grup :::Help Shortcut Key:::", null, null);
        w.center();
        w.setDraggable(true);
//        w.setStyleName("dialog");
        getUI().addWindow(w);
