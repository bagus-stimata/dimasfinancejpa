...
public class BkktranstypePresenter implements ClickListener, ValueChangeListener, Handler {
	/**
	 * 
	 */
	...
	private BkktranstypeView view;
	
	
	public BkktranstypePresenter(BkktranstypeModel model, BkktranstypeView view){
		this.model = model;
		this.view = view;
		initListener();		
		initDisplay();
	...
	protected void addItem() {
		try {
			...
			model.newItem = new Bkktranstype();  
			...
			model.getBinderBkktranstype().setItemDataSource(model.newItem);
	...
	public int updateAndValidateForm(){
		try{
			//1. Commit form
			try{
				model.getBinderBkktranstype().commit();
			} catch(Exception ex){
...
//		}
		...
		if (model.getBkktranstypeService().findAllById(theId).size()>0){
			kodeValid = false;
	                ...
	                new ConfirmDialog.Listener() {
	                    public void onClose(ConfirmDialog dialog) {
	                        if (dialog.isConfirmed()) {
	                    		...
	                    		try {			
	                    			//1. form discard
	                    			model.getBinderBkktranstype().discard();
	                    			view.setOperationStatus(EnumFormOperationStatus.OPEN.getStrCode());
	                    			view.setFormButtonAndText();
	                    			//2. refresh container
	                        		model.setFreshDataTable();
...
//	}
	...
	public void valueChange(ValueChangeEvent event) {
		...
		if (entitySelected) {
			...
			model.item = new Bkktranstype();
			model.item = model.getTableJpaContainer().getItem(itemId).getEntity();
			model.getBinderBkktranstype().setItemDataSource(model.item);
