...
public class AccountPresenter implements ClickListener, ValueChangeListener, Handler {
	/**
	 * 
	 */
...
//			discardForm(operationStatus);
	...
	protected void addItem() {
		try {
			...
			item.setId("New");
			item.setName("");
			item.setActualbalance(0.0);
			item.setTipedebetkredit("");
			
			//2. Tampilkan form
			model.formBeanItem = new BeanItem<Account>(item);
