...
public class SalesmanPresenter implements ClickListener, ValueChangeListener, Handler {
	/**
	 * 
	 */
...
//			discardForm(operationStatus);
	...
	protected void addItem() {
		try {
			...
			model.newItem = new Salesman();  
			//Karena tidak pisa pake null representation
			SalesmanPK id = new SalesmanPK();
			id.setSpcode("NEW");	
			model.newItem.setId(id);
			
			model.newItem.setSpname("");
			model.newItem.setSalestype("");
			
			//2. SET NEW DATA ITEM TO BINDER
			model.getBinderSalesman().setItemDataSource(model.newItem);
	...
	public int updateAndValidateForm(){
		try{
			//1. Commit form
			model.getBinderSalesman().commit();
	...
	public int insertAndValidateForm(){
		...
		String theId = (String) view.getFieldSpcode().getConvertedValue();
		String strDivision = "";
		...
		if (model.getSalesmanService().findAllById(theId, strDivision).size()>0){
			kodeValid = false;
	                ...
	                new ConfirmDialog.Listener() {
	                    public void onClose(ConfirmDialog dialog) {
	                        if (dialog.isConfirmed()) {
	                    		...
	                    		try {			
	                    			//1. form discard
	                    			model.getBinderSalesman().discard();
	                    			view.setOperationStatus(EnumFormOperationStatus.OPEN.getStrCode());
	                    			view.setFormButtonAndText();
	                    			//2. refresh container
	                        		model.setFreshDataTable();
...
//	                        	view.getForm().getField("spcode").focus();
	...
	public int searchForm(){
		...
		Filter filter1 = new SimpleStringFilter("id.spcode", theId, true, false);
		Filter filter2 = new Or(new SimpleStringFilter("spname", theName, true, false));
...
//	}
	...
	public void valueChange(ValueChangeEvent event) {
		...
		if (entitySelected) {
			...
			model.item = new Salesman();
			model.item = model.getTableJpaContainer().getItem(itemId).getEntity();
			model.getBinderSalesman().setItemDataSource(model.item);
