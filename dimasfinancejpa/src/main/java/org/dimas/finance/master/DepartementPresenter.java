...
public class DepartementPresenter implements ClickListener, ValueChangeListener, Handler {
	/**
	 * 
	 */
	...
	public DepartementPresenter(DepartementModel model, DepartementView view){
		this.model = model;
		this.view = view;
		initListener();		
		initDisplay();
...
//			discardForm(operationStatus);
	...
	protected void addItem() {
		try {
			...
			item.setDepartement("");
			//2. Tampilkan form
			model.formBeanItem = new BeanItem<Departement>(item);
	...
	public int searchForm(){
		...
		Filter filter2 = new Or(new SimpleStringFilter("departement", theName, true, false));
