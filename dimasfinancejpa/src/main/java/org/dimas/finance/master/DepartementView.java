...
public class DepartementView extends CustomComponent {
	private DepartementModel model;
	private VerticalLayout content = new VerticalLayout();

//	private JPAContainer<Departement> container;
	...
	private Class<Departement> entityClass;
...
//	private BeanItem<AccountGrup> beanItemAccountGrup;
	...
	public DepartementView(DepartementModel model){
		this.model = model;		
		initContainer();
		initFieldFactory();
		buildView();
...
//		table.addActionHandler(this);		
	...
	public void setTableProperties(){
		setVisibleTableProperties("id","departement");
		//set header
		table.setColumnHeader("id", "ID");
		table.setColumnHeader("departement", "DEPARTMENT");
		
		table.setColumnExpandRatio("id", 1);
		table.setColumnExpandRatio("departement", 3);		
	}
	public void setFormProperties(){
		setVisibleFormProperties("id", "departement");
	}
...
//			final JPAContainer<AccountGrup> grupJPAContainer = JPAContainerFactory.make(AccountGrup.class, PERSISTENCE_UNIT);
			...
			if ("id".equals(propertyId)) {
                ...
                field.addValidator(new StringLengthValidator("ID antar 1 s/d 6", 1, 6, false));
				return field;
			} else if ("departement".equals(propertyId)) {
				field = new TextField("DEPARTMENT: ");
				field.setRequired(true);
				field.setRequiredError("Department tidak boleh kosong");
...
//			}
	...
	public DepartementModel getModel() {
		return model;
	}
	public void setModel(DepartementModel model) {
		this.model = model;
	}
