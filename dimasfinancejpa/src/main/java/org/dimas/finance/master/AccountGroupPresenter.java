...
public class AccountGroupPresenter implements ClickListener, ValueChangeListener, Handler {
	/**
	 * 
	 */
	...
	Accountgroup item = new Accountgroup();
	
	public AccountGroupPresenter(AccountGroupModel model, AccountGroupView view){
		this.model = model;
		this.view = view;
		initListener();		
		initDisplay();
...
//			discardForm(operationStatus);
	...
	protected void addItem() {
		try {
			...
			//2. Tampilkan form
			model.formBeanItem = new BeanItem<Accountgroup>(item);
