...
public class AccountGroupReportPresenter implements ClickListener, ValueChangeListener, Handler {
	/**
	 * 
	 */
	...
	private AccountGroupReportView view;
	
	Accountgroupreport item = new Accountgroupreport();
	
	public AccountGroupReportPresenter(AccountGroupReportModel model, AccountGroupReportView view){
		this.model = model;
		this.view = view;
		initListener();		
		initDisplay();
...
//			discardForm(operationStatus);
	...
	protected void addItem() {
		try {
			...
			item = new Accountgroupreport();  
			//Karena tidak pisa pake null representation
			item.setId("New");
			item.setName("");
			//2. Tampilkan form
			model.formBeanItem = new BeanItem<Accountgroupreport>(item);
