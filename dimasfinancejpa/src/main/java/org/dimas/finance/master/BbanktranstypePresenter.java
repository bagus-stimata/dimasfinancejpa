...
public class BbanktranstypePresenter implements ClickListener, ValueChangeListener, Handler {
	/**
	 * 
	 */
	...
	private BbanktranstypeView view;
	
	
	public BbanktranstypePresenter(BbanktranstypeModel model, BbanktranstypeView view){
		this.model = model;
		this.view = view;
		initListener();		
		initDisplay();
	...
	protected void addItem() {
		try {
			...
			model.newItem = new Bbanktranstype();  
			...
			model.getBinderBbanktranstype().setItemDataSource(model.newItem);
	...
	public int updateAndValidateForm(){
		try{
			//1. Commit form
			try{
				model.getBinderBbanktranstype().commit();
			} catch(Exception ex){
...
//		}
		...
		if (model.getBbanktranstypeService().findAllById(theId).size()>0){
			kodeValid = false;
	                ...
	                new ConfirmDialog.Listener() {
	                    public void onClose(ConfirmDialog dialog) {
	                        if (dialog.isConfirmed()) {
	                    		...
	                    		try {			
	                    			//1. form discard
	                    			model.getBinderBbanktranstype().discard();
	                    			view.setOperationStatus(EnumFormOperationStatus.OPEN.getStrCode());
	                    			view.setFormButtonAndText();
	                    			//2. refresh container
	                        		model.setFreshDataTable();
...
//	}
	...
	public void valueChange(ValueChangeEvent event) {
		...
		if (entitySelected) {
			...
			model.item = new Bbanktranstype();
			model.item = model.getTableJpaContainer().getItem(itemId).getEntity();
			model.getBinderBbanktranstype().setItemDataSource(model.item);
