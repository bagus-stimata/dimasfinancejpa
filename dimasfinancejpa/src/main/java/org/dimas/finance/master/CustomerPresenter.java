...
public class CustomerPresenter implements ClickListener, ValueChangeListener, Handler {
	/**
	 * 
	 */
	...
	public void buttonClick(ClickEvent event) {
		...
		if (event.getButton() == view.getAddButton()) {
			if (view.getOperationStatus().equals(EnumFormOperationStatus.OPEN.getStrCode())){
				addItem();
//				if (view.getForm().isVisible()) view.getForm().getField("custno").focus();
...
//			discardForm(operationStatus);
	...
	protected void addItem() {
		try {
			...
			model.newItem = new Customer();  
			//Karena tidak pisa pake null representation
//			item.setCustno("New");
			CustomerPK id = new CustomerPK();
			id.setCustno("New");
			model.newItem.setId(id);
			
			model.newItem.setCustname("");
			model.newItem.setAddress1("");
			model.newItem.setAddress2("");
			model.newItem.setCity1("");
			model.newItem.setCity2("");
			model.newItem.setContactperson("");
			model.newItem.setCreditlimit(0.0);
			model.newItem.setCrterm(0);
			model.newItem.setHp("");
			model.newItem.setJoindate(new Date());
			model.newItem.setLasttrans(new Date());
			model.newItem.setLastupdate(new Date());
			model.newItem.setPhone("");
			model.newItem.setPostcode("");
			model.newItem.setState("");
			model.newItem.setTunaikredit("");
			model.newItem.setTypecustomer("");
			
			//2. SET NEW DATA ITEM TO BINDER
			model.getBinderCustomer().setItemDataSource(model.newItem);
	...
	public int updateAndValidateForm(){
		try{
			//1. Commit form
			model.getBinderCustomer().commit();
	...
	public int insertAndValidateForm(){
		...
		String theId = (String) view.getFieldCustno().getConvertedValue();
		String strDivision = "";
		...
		if (model.getCustomerService().findAllById(theId, strDivision).size()>0){
			kodeValid = false;
	                ...
	                new ConfirmDialog.Listener() {
	                    public void onClose(ConfirmDialog dialog) {
	                        if (dialog.isConfirmed()) {
	                    		...
	                    		try {			
	                    			//1. form discard
	                    			model.getBinderCustomer().discard();
	                    			view.setOperationStatus(EnumFormOperationStatus.OPEN.getStrCode());
	                    			view.setFormButtonAndText();
	                    			//2. refresh container
	                        		model.setFreshDataTable();
	...
	public int searchForm(){
		...
		Filter filter1 = new SimpleStringFilter("id.custno", theId, true, false);
		Filter filter2 = new Or(new SimpleStringFilter("custname", theName, true, false));
...
//	}
	...
	public void valueChange(ValueChangeEvent event) {
		...
		if (entitySelected) {
			...
			model.item = new Customer();
			model.item = model.getTableJpaContainer().getItem(itemId).getEntity();
			model.getBinderCustomer().setItemDataSource(model.item);
