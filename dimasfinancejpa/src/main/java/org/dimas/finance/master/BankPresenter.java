...
public class BankPresenter implements ClickListener, ValueChangeListener, Handler {
	/**
	 * 
	 */
	...
	public void buttonClick(ClickEvent event) {
		...
		if (event.getButton() == view.getAddButton()) {
			if (view.getOperationStatus().equals(EnumFormOperationStatus.OPEN.getStrCode())){
				addItem();
				if (view.getForm().isVisible()) view.getForm().getField("idbank").focus();
...
//			discardForm(operationStatus);
	...
	public void setFormButtonAndText(){
		if (view.getOperationStatus().equals(EnumFormOperationStatus.OPEN.getStrCode())){
			...
			try{
				view.getForm().getField("idbank").setReadOnly(true);
			} catch(Exception ex){
			}
		} else if (view.getOperationStatus().equals(EnumFormOperationStatus.ADDING.getStrCode())){
			...
			try{
				view.getForm().getField("idbank").setReadOnly(false);
			} catch(Exception ex){}
		}else if (view.getOperationStatus().equals(EnumFormOperationStatus.EDITING.getStrCode())){
			view.getForm().setVisible(true);
			view.getTable().setSelectable(true);
	...
	protected void addItem() {
		try {
			...
			//2. Tampilkan form
			model.formBeanItem = new BeanItem<Bank>(item);
	...
	public int insertAndValidateForm(){
		...
		String theId = view.getForm().getField("idbank").getValue().toString();
		//1. Validasi Kode
		...
		Equal filter = new Compare.Equal("idbank", theId);
	                ...
	                new ConfirmDialog.Listener() {
	                    public void onClose(ConfirmDialog dialog) {
	                        if (dialog.isConfirmed()) {
	                        	...
	                        	view.getForm().getField("idbank").focus();
	                            // User did not confirm
	                        }
	                    }
	                });	        
