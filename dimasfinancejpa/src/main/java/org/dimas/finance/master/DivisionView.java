...
//	private JPAContainer<Division> container;
	...
	private Class<Division> entityClass;
...
//		table.addActionHandler(this);		
	...
	public void setTableProperties(){
		setVisibleTableProperties("id","divisi", "areaBean", "departementBean");
		//set header
		table.setColumnHeader("id", "ID");
		table.setColumnHeader("divisi", "DIVISI");
		table.setColumnHeader("areaBean", "AREA");
		table.setColumnHeader("departementBean", "DEPARTEMENT");
		
		table.setColumnExpandRatio("id", 1);
		table.setColumnExpandRatio("divisi", 2);		
		table.setColumnExpandRatio("areaBean", 3);		
		table.setColumnExpandRatio("departementBean", 3);		
	}
	public void setFormProperties(){
		setVisibleFormProperties("id", "divisi", "areaBean", "departementBean");
	}
...
//		}
    ...
    protected static class CustomFieldFactory extends DefaultFieldFactory {
		public Field createField(Item item, Object propertyId, Component uiContext){
			final JPAContainer<Area> areaJPAContainer = JPAContainerFactory.make(Area.class, "financePU");
			final JPAContainer<Departement> departementJPAContainer = JPAContainerFactory.make(Departement.class, "financePU");
			...
			if ("id".equals(propertyId)) {
				...
				return field;
			} else if ("divisi".equals(propertyId)) {
				field = new TextField("DIVISI: ");
				field.setRequired(true);
				field.setRequiredError("Divisi tidak boleh kosong");
...
//				field.addValidator(new BeanValidator(Person.class, "name"));	
				...
				return field;				
			} else if ("areaBean".equals(propertyId)) {
				//DARI JPA CONTAINER KE ARRAY LIST
				List<Area> list = new ArrayList<Area>();
				Collection itemIds = areaJPAContainer.getItemIds();
				for (Object object : itemIds) {
	                EntityItem entityItem = areaJPAContainer.getItem(object);
	                Object entity = entityItem.getEntity();
	                list.add((Area) entity);
	            }

//				ComboBox c = new ComboBox("Combobox: ");					
				ComboBox c = new ComboBox("AREA: ", list);					
				...
				return c;
			} else if("departementBean".equals(propertyId)){
				//DARI JPA CONTAINER KE ARRAY LIST
				List<Departement> list = new ArrayList<Departement>();
				Collection itemIds = departementJPAContainer.getItemIds();
				for (Object object : itemIds) {
	                EntityItem entityItem = departementJPAContainer.getItem(object);
	                Object entity = entityItem.getEntity();
	                list.add((Departement) entity);
	            }

//				ComboBox c = new ComboBox("Combobox: ");					
				ComboBox c = new ComboBox("DEPARTMENT: ", list);					
