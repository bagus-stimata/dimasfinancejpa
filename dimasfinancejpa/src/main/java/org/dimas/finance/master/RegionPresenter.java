...
public class RegionPresenter implements ClickListener, ValueChangeListener, Handler {
	/**
	 * 
	 */
	...
	public RegionPresenter(RegionModel model, RegionView view){
		this.model = model;
		this.view = view;
		initListener();		
		initDisplay();
...
//			discardForm(operationStatus);
	...
	protected void addItem() {
		try {
			...
			item.setRegion("");
			//2. Tampilkan form
			model.formBeanItem = new BeanItem<Region>(item);
