...
public class BkbtranstypeView extends CustomComponent {
	...
	public BkbtranstypeView(BkbtranstypeModel model){
		this.model = model;		
		initContainer();
		initFieldFactory();
		buildView();
...
//		setFormProperties();
	...
	public void bindAndBuildFieldGroupComponent(){
		...
		comboGrup.setItemCaption("GRUP2", "Penerimaan Kas Besar");
		comboGrup.addItem("GRUP3");
		comboGrup.setItemCaption("GRUP3", "Pengeluaran Kas Besar");
		...
		model.getBinderBkbtranstype().bind(fieldId, "id");
		model.getBinderBkbtranstype().bind(fieldDescription, "description");
		model.getBinderBkbtranstype().bind(comboGrup, "grup");
		model.getBinderBkbtranstype().bind(comboAccount, "accountBean");
...
//		addButton.setEnabled(false);
	...
	public BkbtranstypeModel getModel() {
		return model;
	}
	public void setModel(BkbtranstypeModel model) {
		this.model = model;
	}
