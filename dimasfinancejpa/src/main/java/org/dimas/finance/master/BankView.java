...
public class BankView extends CustomComponent {
	private BankModel model;
	private VerticalLayout content = new VerticalLayout();

//	private JPAContainer<Bank> container;
	...
	private Class<Bank> entityClass;
...
//	private BeanItem<AccountGrup> beanItemAccountGrup;
	...
	public BankView(BankModel model){
		this.model = model;		
		initContainer();
		initFieldFactory();
		buildView();
...
//		form.setFormFieldFactory(new FieldFactory());
	...
	public void setTableProperties(){
		setVisibleTableProperties("idbank","description", "rekening",  "gl");
		//set header
		table.setColumnHeader("idbank", "ID BANK");
		table.setColumnHeader("description", "NAMA BANK");
		table.setColumnHeader("rekening", "NO. REK");
		table.setColumnHeader("gl", "GL");
		
		table.setColumnExpandRatio("idbank", 1);
		table.setColumnExpandRatio("description", 2);		
		table.setColumnExpandRatio("gl", 3);		
	}
	public void setFormProperties(){
		setVisibleFormProperties("idbank", "description", "rekening", "gl");
	}
...
//			final JPAContainer<AccountGrup> grupJPAContainer = JPAContainerFactory.make(AccountGrup.class, PERSISTENCE_UNIT);
			...
			Field field = null;
			if ("idbank".equals(propertyId)) {
				field = new TextField("ID BANK: ");
				field.setRequired(true);
				field.setRequiredError("ID BANK tidak boleh Kosong");
                field.addValidator(new StringLengthValidator("ID BANK antar 1 s/d 6", 1, 6, false));
				return field;
			} else if ("description".equals(propertyId)) {
				field = new TextField("DESKRIPSI: ");
				field.setRequired(true);
				field.setRequiredError("Nama Bank tidak boleh kosong");
				
				return field;				
			} else if ("rekening".equals(propertyId)) {
				field = new TextField("NO. REK: ");
				
				return field;				
			} else if ("gl".equals(propertyId)) {
				CheckBox check = new CheckBox("Transfer ke G/L?");
				return check;								
