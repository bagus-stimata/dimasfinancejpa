...
public class PenerimaanBankItemAddView extends CustomComponent {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;

	private PenerimaanBankItemAddModel model;
...
//	private TextField fieldSearchByTitle = new TextField("TITLE");
	...
	public PenerimaanBankItemAddView(PenerimaanBankItemAddModel model){
		this.model = model;
		initComponent();
		buildView();
...
//		formLayoutHorizontal.addComponent(btnClose);
	...
	public void bindAndBuildFieldGroupComponent(){
		...
		model.getBinderItemDetail().bind(comboDivision, "divisionBean");
		model.getBinderItemDetail().bind(fieldDescription, "description");
		model.getBinderItemDetail().bind(comboSalesman, "salesmanBean");
		model.getBinderItemDetail().bind(comboCustomer, "customerBean");
		model.getBinderItemDetail().bind(comboTranstype, "bbanktranstypeBean");
		model.getBinderItemDetail().bind(comboAccount, "accountBean");
		model.getBinderItemDetail().bind(comboDebetkredit, "debetkredit");
		model.getBinderItemDetail().bind(fieldAmount, "amount");
		model.getBinderItemDetail().bind(checkGl, "gl");
	...
	public PenerimaanBankItemAddModel getModel() {
		return model;
	}
	public void setModel(PenerimaanBankItemAddModel model) {
		this.model = model;
	}
