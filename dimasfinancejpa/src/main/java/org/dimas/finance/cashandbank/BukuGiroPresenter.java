...
public class BukuGiroPresenter implements ClickListener, ValueChangeListener{
	private BukuGiroModel model;
	private BukuGiroView view;
	
	public BukuGiroPresenter(BukuGiroModel model, BukuGiroView view){
		this.model = model;
		this.view = view;
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getBeanItemContainerBukugiro().removeItem(itemId);
...
//		model.getBeanItemContainerBukugiroSearch().removeContainerFilter(filter);
		...
		if (kodeValid && inputValid) {
			try {
				...
				BukugiroPK id = new BukugiroPK();
				id.setRefno(model.getTransaksiHelper().getNewNomorUrutBukuGiro(div));
				id.setDivision(strDivision);
				model.getNewBukuGiro().setId(id);
...
//		model.getBeanItemContainerBukugiro().addContainerFilter(filter1);
		...
		if (! theDivision.trim().equals("")){			
			...
			model.getBeanItemContainerBukugiro().addContainerFilter(filter4);
		}
