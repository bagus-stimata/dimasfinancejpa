...
public class PenerimaanKasBesarView extends CustomComponent{
	...
	public PenerimaanKasBesarView(PenerimaanKasBesarModel model){
		this.model = model;
		initComponent();
		buildView();
...
//		model.getBeanItemContainerBkbDetail().sort(new Object[] { "debetkredit"}, new boolean[] { true });
		tableDetail.setContainerDataSource(model.getBeanItemContainerBkbDetail());
	...
	public void setTableDetailProperties(){
		...
		setVisibleTableDetailProperties("selected", "urut", "id","description", "accountBean", 
				"bkbtranstypeBean",  "debetkredit",
				"amount", "custname", "gl", "bkbheaderBean", "divisionBean", "customerBean");
...
//			tableDetail.setColumnCollapsed("bkbtranstypeBean", true);
			tableDetail.setColumnCollapsed("bkbheaderBean", true);
		...
		tableDetail.setColumnHeader("bkbtranstypeBean", "TIPE TRANSAKSI");
...
//		table.setColumnExpandRatio("recapno", 3);
	...
	public void bindAndBuildFieldGroupComponent(){
		...
		model.getBinderBkbheader().bind(fieldRefno, "id.refno");
		model.getBinderBkbheader().bind(comboDivision, "divisionBean");
		model.getBinderBkbheader().bind(dateFieldEntryDate, "entrydate");
		model.getBinderBkbheader().bind(dateFieldTransDate, "transdate");
		model.getBinderBkbheader().bind(fieldNotes, "notes");
		model.getBinderBkbheader().bind(checkClosing, "closing");
	...
	private PenerimaanKBHeaderSelectModel headerSelectModel; 
	private PenerimaanKBHeaderSelectView headerSelectView;
	...
	public void buildWindowHeaderSelect(){
		...
		headerSelectModel = new PenerimaanKBHeaderSelectModel();
		headerSelectView = new PenerimaanKBHeaderSelectView(headerSelectModel);
		
		PenerimaanKBHeaderSelectPresenter modulTempHeaderSelectPresenter = new PenerimaanKBHeaderSelectPresenter(
	...
	private PenerimaanKBItemAddModel itemAddModel; 
	private PenerimaanKBItemAddView itemAddView;
	private PenerimaanKBItemAddPresenter itemAddPresenter;
	...
	public void buildWindowItemAdd(String operationStatus){
		...
		//INITIAL DATA TO PASS		
		itemAddModel = new PenerimaanKBItemAddModel();
		itemAddModel.bkbheader = model.getBkbheader();
		itemAddModel.bkbdetail = model.getBkbdetail();	
		itemAddModel.setOperationStatus(operationStatus);
		if (operationStatus.equals(EnumFormOperationStatus.ADDING.getStrCode())){
			Bkbdetail newBkbdetail = new Bkbdetail();
			itemAddModel.bkbdetail = newBkbdetail;
		}
		itemAddModel.getBinderBkbdetail().setItemDataSource(itemAddModel.getBkbdetail());
		itemAddView = new PenerimaanKBItemAddView(itemAddModel);
		
		itemAddPresenter = new PenerimaanKBItemAddPresenter(
...
//		fieldUserId.setReadOnly(true);
		...
		try{
			if (model.getOperationStatus().equals(EnumFormOperationStatus.ADDING.getStrCode())){
				div = model.getNewBkbheader().getDivisionBean();
			} else {
				div = model.getBkbheader().getDivisionBean();
			}
			strDiv = div.getId();
		} catch(Exception ex){}
		...
		try{
			if (model.getOperationStatus().equals(EnumFormOperationStatus.ADDING.getStrCode())){
				refno = model.getNewBkbheader().getId().getRefno().toString().trim();
			} else {
				refno = model.getBkbheader().getId().getRefno().toString().trim();				
				strDiv = div.getId();
			}
		} catch(Exception ex){}
...
//		System.out.println( model.getOperationStatus() + "\t" + comboDivision.getValue() + "\t" + div + "\t" + strDiv + "\t" + refno);
	...
	public PenerimaanKasBesarModel getModel() {
		return model;
	}

	public void setModel(PenerimaanKasBesarModel model) {
		this.model = model;
	}
	...
	public PenerimaanKBHeaderSelectModel getHeaderSelectModel() {
		return headerSelectModel;
	}

	public void setHeaderSelectModel(PenerimaanKBHeaderSelectModel headerSelectModel) {
		this.headerSelectModel = headerSelectModel;
	}

	public PenerimaanKBHeaderSelectView getHeaderSelectView() {
		return headerSelectView;
	}

	public void setHeaderSelectView(PenerimaanKBHeaderSelectView headerSelectView) {
	...
	public PenerimaanKBItemAddModel getItemAddModel() {
		return itemAddModel;
	}

	public void setItemAddModel(PenerimaanKBItemAddModel itemAddModel) {
		this.itemAddModel = itemAddModel;
	}

	public PenerimaanKBItemAddView getItemAddView() {
		return itemAddView;
	}

	public void setItemAddView(PenerimaanKBItemAddView itemAddView) {
		this.itemAddView = itemAddView;
	}

	public PenerimaanKBItemAddPresenter getItemAddPresenter() {
		return itemAddPresenter;
	}

	public void setItemAddPresenter(PenerimaanKBItemAddPresenter itemAddPresenter) {
