...
//				model.getArPaymentHeader().setRefno(newNomorUrut);
				...
				model.getArPaymentHeader().setDivisionBean(divisionBean);
				model.getArPaymentHeader().setEntrydate(new Date());
				model.getArPaymentHeader().setTransdate(model.getTransaksiHelper().getCurrentTanggalTransaksiBerjalan(divisionBean));
...
//###						newTransfer = model.getBukuTransferService().findById(model.getArPaymentDetail().getBukutransferBean().getRefno());	
			...
			try{
				...
				System.err.println("Binder paymentdetail dan binder arINvoice gagal commit");
			}
