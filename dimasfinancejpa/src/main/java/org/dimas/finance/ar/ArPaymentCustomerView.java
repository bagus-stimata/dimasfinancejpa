...
import com.vaadin.client.ui.VScrollTable.TableFooter;
...
public class ArPaymentCustomerView extends CustomComponent {
...
//	private CustomFieldFactory customFieldFactory;
	...
	private Class<Arpaymentheader> entityClass;
...
//	private TextField fieldPO;
	...
	public ArPaymentCustomerView(ArPaymentCustomerModel model){
		this.model = model;		
...
//		layoutTop.addComponent(fieldPO);
	...
	public void setTablePropertiesHeader(){
		setVisibleTableHeaderProperties("refno","transdate", "entrydate", "notes", "closing");	
		//HEADER
		tableHeader.setColumnHeader("refno", "REFNO");
	...
	public void setTablePropertiesDetail(){
		setVisibleTableDetailProperties("id", "cashamountpay", "returamountpay",
				"giroamountpay", "returBean", "bukugiroBean", "transferamountpay", "bukutransferBean", "potonganamount", "divisionBean",
				"invoiceretur", "arpaymentheaderBean", "invoicetype", "subtotalpay");
		//Collapsing Column
...
//		setVisibleFormProperties("refno", "userid");
	...
	public void setDisplayHeader(){
		...
		if (model.getItemInvoice() !=null){		
			...
			fieldInvoiceNo.setValue(model.getItemInvoice().getId().getInvoiceno());
			fieldIdCustomer.setValue(model.getItemInvoice().getCustomerBean().getId().getCustno());
			fieldNamaCustomer.setValue(model.getItemInvoice().getCustname());
			...
			String strAmount = "0";
			try{
				strAmount = String.valueOf(nf.format(model.getItemInvoice().getAmount()));
			} catch(Exception ex){}
...
//			item = entityItem.getEntity();
			...
			sumReturAmount += item.getReturamountpay();
			sumPotonganAmount += item.getPotonganamount();
	...
	private ArPaymentCustPembayaranModel formModel = new ArPaymentCustPembayaranModel();
	private ArPaymentCustPembayaranView formView = new ArPaymentCustPembayaranView(formModel);
	...
	public void buildFormPembayaran(){
		...
		windowFormPembayaran.setWidth("700px");
		windowFormPembayaran.setHeight("600px");
		
		//Inisial Data
		Arinvoice invoice=new Arinvoice();
		invoice = model.getItemInvoice();
		Arpaymentdetail detail=new Arpaymentdetail();
		detail = model.getItemDetail();
		
		formModel = new ArPaymentCustPembayaranModel(invoice, detail);
		...
		formView = new ArPaymentCustPembayaranView(formModel);
		ArPaymentCustPembayaranPresenter formPresenter = new ArPaymentCustPembayaranPresenter(formModel, formView);		
...
//		getUI().removeWindow(windowFormPembayaran);
	...
	public ArPaymentCustomerModel getModel() {
		return model;
	}
	public void setModel(ArPaymentCustomerModel model) {
		this.model = model;
	}
	...
	public Class<Arpaymentheader> getEntityClass() {
		return entityClass;
	}
	public void setEntityClass(Class<Arpaymentheader> entityClass) {
	...
	public ArPaymentCustPembayaranModel getFormModel() {
		return formModel;
	}
	public void setFormModel(ArPaymentCustPembayaranModel formModel) {
		this.formModel = formModel;
	}
	public ArPaymentCustPembayaranView getFormView() {
		return formView;
	}
	public void setFormView(ArPaymentCustPembayaranView formView) {
