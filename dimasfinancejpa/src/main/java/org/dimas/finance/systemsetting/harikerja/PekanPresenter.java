...
import org.dimas.finance.model.SysCalenderPekan;
...
public class PekanPresenter implements ClickListener, ValueChangeListener{
	private PekanModel model;
	private PekanView view;
	
	public PekanPresenter(PekanModel model, PekanView view){
		this.model = model;
		this.view = view;
	...
	public void fillTableDetail(Division division){
		List<SysCalender> listSysCalender = new ArrayList<SysCalender>();		
		listSysCalender =  model.getSysCalenderService().findAllByDivision(division);
		
		model.getBeanItemContainerPekan().removeAllContainerFilters();
		model.getBeanItemContainerPekan().removeAllItems();
		
		List<SysCalenderPekan> listPekan = new ArrayList<SysCalenderPekan>();
		int pekan;
		for (SysCalender sysCalender: listSysCalender){
			
		}
		
		
		model.getBeanItemContainerPekan().addAll(listPekan);
	
		view.getTable().setContainerDataSource(model.getBeanItemContainerPekan());
		view.getTable().refreshRowCache();
